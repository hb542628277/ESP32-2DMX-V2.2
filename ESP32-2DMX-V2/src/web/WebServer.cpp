...
void WebServer::loadAPConfig() {
    ...
    if (!file) {
        // 使用默认配置
        strlcpy(apConfig.ssid, "ESP32-DMX-AP", sizeof(apConfig.ssid));
        strlcpy(apConfig.password, "12345678", sizeof(apConfig.password));
    ...
    if (!error) {
        strlcpy(apConfig.ssid, doc["ssid"] | "ESP32-DMX-AP", sizeof(apConfig.ssid));
        strlcpy(apConfig.password, doc["password"] | "12345678", sizeof(apConfig.password));
        apConfig.enabled = doc["enabled"] | false;
    }
}
